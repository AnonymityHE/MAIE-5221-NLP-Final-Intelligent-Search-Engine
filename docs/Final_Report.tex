...
\label{tab:examples}
\small
\begin{tabular}{@{}p{0.28\textwidth}p{0.1\textwidth}p{0.48\textwidth}@{}}
\toprule
\textbf{Query} & \textbf{Tool} & \textbf{Response Summary} \\ \midrule
``香港科技大学在哪里？'' & local\_rag & ``位于中国香港特别行政区'' (9.58s) \\
``苹果公司的股价是多少？'' & finance & ``当前股价是\$271.11'' (8.98s) \\
``比较香港和北京今天的天气'' & weather & Detailed comparison: HK 21°C, Beijing -1°C (21.06s) \\
``比亚迪和特斯拉哪个股价更高？'' & finance & ``特斯拉(\$386.39)比比亚迪(\$92.70)更高'' (11.61s) \\
``RAG系统的核心组件有哪些？'' & local\_rag & Listed 5 core components from knowledge base (11.00s) \\ \bottomrule
\end{tabular}
\end{table}
